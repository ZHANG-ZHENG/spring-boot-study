...
public class CommodityServiceImpl implements CommodityService {
	...
	public List<Commodity> getByName(String name) {
		...
		MatchQueryBuilder matchQueryBuilder = new MatchQueryBuilder("name",
				name);
		Iterable<Commodity> iterable = commodityRepository
				.search(matchQueryBuilder);
		// iterable.forEach(e->list.add(e));
		
		iterable.forEach(new Consumer<Commodity>() {
			@Override
			public void accept(Commodity t) {
				list.add(t);
			}
		});
